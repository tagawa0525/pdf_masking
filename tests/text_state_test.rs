...
fn test_empty_content() {
    let content = b"";
    let result = parse_content_operations(content, None).expect("should handle empty");
...
fn test_no_text_blocks() {
    ...
    let result = parse_content_operations(content, None).expect("should handle no text");
    assert!(result.text_commands.is_empty());
    assert!(!result.non_text_operations.is_empty());
}
