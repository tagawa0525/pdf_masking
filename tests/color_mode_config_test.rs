...
// ============================================================
// 6. 後方互換性: pages フィールドが存在する場合はエラー
// ============================================================

#[test]
#[ignore] // pages フィールドは削除され、コンパイル時に存在しないことが保証される
fn test_pages_field_deprecated_error() {
    // pagesフィールドは Job 構造体から削除されたため、
    // コンパイル時に存在しないことが保証される。
    // 実行時テストは不要。
}
