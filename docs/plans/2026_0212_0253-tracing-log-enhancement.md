...
この変更前は `tracing` は4ファイル（main.rs, job_runner.rs, page_processor.rs,
font.rs）でのみ使用されていた。残りの約15モジュールにはログ出力がなく、
本番環境でのデバッグや処理状況の把握が困難。各モジュールに適切なレベルの
ログを追加し、パイプライン全体の可視性を高める。
